...
router.get("/", async (req, res) => {
  ...
  if (name) {
    if (dietsup.indexOf(name) === -1) {
      ...
      ); //si incluye el nombre que viene por query
      recipeName.length
      ...
      let filterDiet = await recipesTotal.filter((recipe) =>
...
router.get("/:id", async (req, res, next) => {
  ...
  if (rId) {
    ...
    try {
      const dbRecipe = await Recipe.findByPk(id, { include: Diets });
      res.json(dbRecipe);
  ...
  //[if you dont have nothing in DB , search in API ]
    ...
    try {
      ...
      let recipe = {
        ...
        pricePerServing: recipeApiID.pricePerServing,
        dishTypes: recipeApiID.dishTypes.map((d) => {
          return { name: d };
        }),
        diets: recipeApiID.diets.map((d) => {
          return { name: d };
        }),
